...
final int sin_freq = 2000;
float [] sin_data = new float [sin_freq];
...
final String [] NN_SERIAL_NAME = {"/dev/pts/21", "/dev/pts/25"};
...
void setup() {
  ...
  try {
    ...
    println("Error opening serial port: " + AR_SERIAL_NAME);
  }
  ...
  for (int i = 0; i < NN_channel; ++i) {
    try {
      ...
      println("Error opening serial port: " + NN_SERIAL_NAME[i]);
    }
  }

  // Generate sin wave
  final float sin_inc = 6.28 / sin_freq;
  for (int i = 0; i < sin_freq; ++i) {
    sin_data[i] = sin(sin_inc * i);
  }
}
